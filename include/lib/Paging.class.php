...
class Paging
{
    ...
    public function __construct($num_rows, $page_num_rows = 10, $page_button_num = 5, $page_name = 'page')
    {
        ...
        $this->all_page_num = intval(ceil(($this->num_rows) / ($this->page_num_rows)));
        //判断按钮数量是否大于了总页数
    ...
    private function get_query_string()
    {
        ...
        foreach ($this->unset_query_array as $value) {
            unset($query_array[$value]);
        }
        unset($query_array[$this->page_name]);
    ...
    public function unset_query()
    {
        $keys = func_get_args();
        if (func_num_args() == 1) {
            $this->unset_query_array = is_array($keys[0]) ? $keys[0] : $keys;
        } else {
    ...
    public function show()
    {
         ...
         */
        if ($this->this_page > 1) {
            $html .= '<li><a href="' . $query_string . $this->page_name . '=1" aria-label="Previous"><span aria-hidden="true">首页</span></a></li>';
            $html .= '<li><a href="' . $query_string . $this->page_name . '=' . ($this->this_page - 1) . '" aria-label="Previous"><span aria-hidden="true">上页</span></a></li>';
        } else {
            $html .= '<li class="disabled"><a href="javascript:void(0);" aria-label="Previous"><span aria-hidden="true">首页</span></a></li>';
            $html .= '<li class="disabled"><a href="javascript:void(0);" aria-label="Previous"><span aria-hidden="true">上页</span></a></li>';
        }
        /**
         * 上一页结束
         */
        ...
        if ($this->this_page < $this->page_button_num && $this->this_page <= (($this->all_page_num - $this->page_button_num) + 1)) {
            $page_num_rows = $this->all_page_num < $this->page_button_num ? $this->all_page_num : $this->page_button_num;
         ...
         * 跳转按钮
         */
        $html .= '<li><form action="" method="get" accept-charset="utf-8" class="form-inline" style="display: inline-block;">';
        foreach ($this->query_array as $key => $value) {
            $html .= '<input type="hidden" name="' . $key . '" value="' . $value . '">';
        }
        $html .= '<input type="text" name="' . $this->page_name . '" class="form-control" value="" placeholder="跳转" style="width: 55px;"><input class="btn btn-default" type="submit" value="翻页"></form></li>';
        $html .= '</ul></nav>';
        return $html;
    }
}
